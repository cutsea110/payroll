fn main() -> Result<(), anyhow::Error> {
    ...
    let tx_factory = TxFactoryImpl {
        ...
        change_method_hold: &|id| Box::new(ChgHoldTx::new(id, db.clone())),
        change_method_direct: &|id, bank, account| {
            Box::new(ChgDirectTx::new(id, bank, account, db.clone()))
        },
        change_method_mail: &|id, address| Box::new(ChgMailTx::new(id, address, db.clone())),
        add_union_member: &|emp_id, member_id, dues| {
            Box::new(ChgMemberTx::new(member_id, emp_id, dues, db.clone()))
        },
        delete_union_member: &|id| Box::new(ChgUnaffiliatedTx::new(id, db.clone())),
