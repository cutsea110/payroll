...
pub struct TxFactoryImpl<'a> {
    ...
    pub change_employee_hold: &'a dyn Fn(EmployeeId) -> Box<dyn Transaction>,
    pub change_employee_direct: &'a dyn Fn(EmployeeId, &str, &str) -> Box<dyn Transaction>,
    pub change_employee_mail: &'a dyn Fn(EmployeeId, &str) -> Box<dyn Transaction>,
    pub change_employee_member: &'a dyn Fn(EmployeeId, MemberId, f32) -> Box<dyn Transaction>,
    pub change_employee_no_member: &'a dyn Fn(EmployeeId) -> Box<dyn Transaction>,
...
impl<'a> TxFactory for TxFactoryImpl<'a> {
    ...
    fn mk_change_employee_direct_tx(
        ...
        (self.change_employee_direct)(id, bank, account)
    }
    fn mk_change_employee_mail_tx(&self, id: EmployeeId, address: &str) -> Box<dyn Transaction> {
        ...
        (self.change_employee_mail)(id, address)
    }
    fn mk_change_employee_member_tx(
        ...
        (self.change_employee_member)(id, member_id, dues)
    }
    fn mk_change_employee_no_member_tx(&self, id: EmployeeId) -> Box<dyn Transaction> {
        ...
        (self.change_employee_no_member)(id)
    }
